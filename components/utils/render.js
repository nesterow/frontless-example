 ...
 * */
module.exports = async function renderAsync(
  tagName,
  component,
  props,
  sharedAttributes
  ...
  try {
    ...
    if (element) {
      ...
      if (element.fetch) await element.fetch(props);

      if (element.onServer) element.onServer(props.req, props.res, props.next);

      state[element.id || component.name] = element.state;
      shared[element.id || component.name] = sharedAttributes.map(name => ({
        name,
        data: element[name]
      }));
